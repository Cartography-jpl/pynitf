...
def test_write_type(isolated_dir):
    ...
    for typ in (np.uint8, np.int8, np.uint16, np.int16, np.uint32, np.int32,
        ...
        img2 = f2.image_segment[0]
        assert img2.data.dtype == np.dtype(typ).newbyteorder('>')
        assert img2.data.shape == (1, nrow, ncol)
        for i in range(nrow):
            for j in range(ncol):
                assert img2.data[i,j] == img.data[i,j]
                ...
                if(typ == np.complex64):
                    ...
                    assert typ(gdal_value("test.ntf", i, j)) == img.data[i,j]
...
def test_write_band(isolated_dir):
    ...
    for b in range(nband):
        for i in range(nrow):
            for j in range(ncol):
                assert img2.data[b, i,j] == img.data[b, i,j]
                assert int(gdal_value("test.ntf", i, j, b)) == img.data[b,i,j]
