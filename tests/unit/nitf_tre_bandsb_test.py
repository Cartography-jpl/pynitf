from pynitf import TreBANDSB
import io
import numpy.testing as npt


def test_tre_bandsb_basic(evil_float1, evil_float2, evil_float3):
    t = TreBANDSB()
    ...
    for i in range(t2.count):
        ...
        npt.assert_almost_equal(t2.lbound[i], evil_float1, 5)
        npt.assert_almost_equal(t2.ubound[i], evil_float2, 5)
...
def test_tre_bandsb_minimum():
    ...
    assert (
        fh.getvalue()
        == b"BANDSB0047600005REFLECTANCE             F?\x80\x00\x00\x00\x00\x00\x009999.99M8888.88M7777.77M6666.66Maaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\x99\x80\x00\x00FOCAL PLANE             \x00\x00\x00\x00U0                                                 110000.010000.01                                                 110000.010000.02                                                 110000.010000.03                                                 110000.010000.04                                                 110000.010000.0"
    )
