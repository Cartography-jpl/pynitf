from pynitf import NitfFile, nitf_file_diff, NitfDesSegment, DesEXT_h5
import json
...
def test_nitf_diff_neq_one_val(isolated_dir):
    ...
    assert not nitf_file_diff("basic_nitf.ntf", "basic2_nitf.ntf")
...
def test_nitf_diff_image_segment_value_tolerance(config_dir):
    ...
    assert nitf_file_diff("basic_nitf.ntf", "basic2_nitf.ntf", config=config_data)
...
def test_nitf_diff_image_segment_histogram_tolerance(config_dir):
    ...
    with open("nitf_diff_histogram_tolerance.json") as fh:
        config_data = json.load(fh)
    assert not nitf_file_diff("basic_nitf.ntf", "basic2_nitf.ntf", config=config_data)
