from pynitf import TreENGRDA, NitfFile
import io
import numpy as np
...
def test_tre_engrda():
    ...
    assert (
        fh.getvalue()
        == b"ENGRDA00125My_sensor           00305TEMP100010001S2tC00000001\x01\x1505TEMP200010001R4tK00000001C\x8a\xb9\x9a10TEMP3 Wall00100001A1NA0000001010.7 DEG C"
    )
...
def test_file_engrda(isolated_dir):
    ...
    assert all(
        t in f2.engrda.keys() for t in ["My_sensor 1", "My_sensor 2", "My_sensor 3"]
    )
    print(f2)
    f3 = NitfFile()
    ...
    assert f4.engrda["My_sensor 1"]["TEMP1"] == (
        np.array(
            [
                [
                    277,
    ...
    assert f4.engrda["My_sensor 1"]["TEMP2"] == (
        np.array(
            [
                [
                    277.45,
