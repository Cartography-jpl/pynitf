from pynitf import NitfFile, TreSTDIDC, NitfDesSegment, DesEXT_h5
...
def test_nitf_diff(isolated_dir, unit_test_data):
    f = NitfFile()
    create_tre(f)
    ...
    t = subprocess.run(["nitf_diff", "basic_nitf.ntf", "basic2_nitf.ntf"])
    assert t.returncode == 1
    print("Results of nitf_diff, should be different. Generates log file")
    t = subprocess.run(
        ["nitf_diff", "--log-file=nitf_diff.log1", "basic_nitf.ntf", "basic2_nitf.ntf"]
    )
    assert t.returncode == 1
    ...
    t = subprocess.run(
        [
            "nitf_diff",
            "--config-file-python="
            + str(unit_test_data / "sample_nitf_diff_config.py"),
            "basic_nitf.ntf",
            "basic2_nitf.ntf",
        ]
    )
...
def test_special_eq_diff(isolated_dir, unit_test_data):
    ...
    f.write("nitf2.ntf")
    print("Results of nitf_diff, should be different")
    t = subprocess.run(["nitf_diff", "nitf1.ntf", "nitf2.ntf"])
    assert t.returncode == 1
    ...
    t = subprocess.run(
        [
            "nitf_diff",
            "--config-file-python="
            + str(unit_test_data / "sample_nitf_diff_config2.py"),
            "nitf1.ntf",
            "nitf2.ntf",
        ]
    )
...
@require_h5py
def test_nitf_diff_h5py(isolated_dir, unit_test_data):
    """Add h5py file"""
    import h5py
    ...
    g.create_dataset(
        "test_data", data=b"hi there", dtype=h5py.special_dtype(vlen=bytes)
    )
    h.close()
    ...
    g.create_dataset(
        "test_data",
        data=b"hi there, this is different",
    ...
    print("Results of nitf_diff, should be same")
    t = subprocess.run(["nitf_diff", "nitf1.ntf", "nitf1.ntf"])
    assert t.returncode == 0
...
def test_extra_tre(isolated_dir, unit_test_data):
