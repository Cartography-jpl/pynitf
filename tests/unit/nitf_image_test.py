...
import io
import numpy as np
from fixtures.support_func import gdal_value, require_gdal_value
import pytest


def test_basic_read(unit_test_data):
    t = NitfFileHeader()
    ...
    for b in range(nband):
        for i in range(nrow):
            for j in range(ncol):
                ...
                assert int(gdal_value("test.ntf", i, j, b)) == 2 * (
                    10 * i + j + b * 100
                )
...
def test_write_data_on_demand(isolated_dir):
    ...
    ncol = 200
    f = NitfFile()
    dn = NitfImageWriteNumpy(nrow, ncol, np.int32, numbands=nband, iid1="DN", idlvl=0)
    for r in range(nrow):
        dn[0, r, :] = np.arange(100 * r, 100 * r + dn.shape[2])
    gain = NitfImageWriteNumpy(
        nrow, ncol, np.float32, numbands=nband, iid1="Gain", idlvl=1
    )
    offset = NitfImageWriteNumpy(
        nrow, ncol, np.float32, numbands=nband, iid1="Offset", idlvl=2
    )
    ...
    rad1 = NitfImageWriteDataOnDemand(
        nrow,
        ncol,
        np.float32,
    ...
    rad2 = NitfImageWriteDataOnDemand(
        nrow,
        ncol,
        np.float32,
        NitfRadCalc(dn, gain, offset).data_to_write,
...
def test_write_on_demand_blocking(isolated_dir):
    ...
    img = NitfImageWriteDataOnDemand(
        nrow=nrow,
        ncol=ncol,
        data_type=np.int32,
        numbands=nband,
        data_callback=write_by_row_p,
...
def test_diff(print_logging):
    ...
    nrow = 10
    ncol = 20
    dn = NitfImageWriteNumpy(nrow, ncol, np.float32, numbands=nband, iid1="DN", idlvl=0)
    for r in range(nrow):
        dn[0, r, :] = np.arange(100 * r, 100 * r + dn.shape[2])
    f = NitfFile()
    f.image_segment.append(NitfImageSegment(dn))
    f.write("file1.ntf")
    dn2 = NitfImageWriteNumpy(
        nrow, ncol, np.float32, numbands=nband, iid1="DN", idlvl=0
    )
    for r in range(nrow):
    ...
    f.image_segment.append(NitfImageSegment(dn2))
    f.write("file3.ntf")
    dn3 = NitfImageWriteNumpy(
        nrow + 1, ncol, np.float32, numbands=nband, iid1="DN", idlvl=0
    )
    ...
    f.write("file4.ntf")
    d = NitfDiff()
    assert d.compare("file1.ntf", "file2.ntf")
    assert not d.compare("file1.ntf", "file3.ntf")
    assert not d.compare("file1.ntf", "file4.ntf")
