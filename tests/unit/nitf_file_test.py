...
from fixtures.support_func import (
    create_image_seg,
    create_tre,
    create_des,
    create_text_segment,
...
def check_tre(t, angle_to_north=270):
    ...
    npt.assert_almost_equal(t.mean_gsd, 105.2)
    assert t.dynamic_range == 2047
    npt.assert_almost_equal(t.obl_ang, 34.12)
    npt.assert_almost_equal(t.roll_ang, -21.15)
    assert t.n_ref == 0
    assert t.rev_num == 3317
    assert t.n_seg == 1
    assert t.max_lp_seg == 6287
    npt.assert_almost_equal(t.sun_el, 68.5)
    npt.assert_almost_equal(t.sun_az, 131.3)
...
def create_sample(unit_test_data):
...
def test_read_tre(unit_test_data):
...
def test_large_tre_write(isolated_dir):
    ...
    check_tre(
        [tre for tre in f2.image_segment[0].tre_list if tre.tre_tag == "USE00A"][0], 290
    )
    check_tre(
        [tre for tre in f3.image_segment[0].tre_list if tre.tre_tag == "USE00A"][0], 290
    )
    print_diag(f2)
    print_diag(f3)
...
def test_copy_worldview(nitf_sample_wv2):
    ...
    f = NitfFile()
    f.data_handle_set.add_handle(NitfDesCopy, priority_order=-999)
    f.read(nitf_sample_wv2)
    original_output = str(f)
    fname_copy = "worldview_copy.ntf"
    f.write(fname_copy)
    f2 = NitfFile()
    f2.data_handle_set.add_handle(NitfDesCopy, priority_order=-999)
    f2.read(fname_copy)
    copy_output = str(f2)
...
def test_full_file_security(isolated_dir, security_fake):
    """This create an end to end NITF file, this was at least initially the
    same as basic_nitf_example.py but as a unit test.
...
@pytest.mark.skip(reason="don't support NITF streaming yet")
...
@pytest.mark.skip(reason="skip profiling by default")
...
def test_too_many_file(isolated_dir):
    ...
    f1 = NitfFile("large_nitf.ntf")  # noqa: F841
    f2 = NitfFile("large_nitf.ntf")  # noqa: F841
...
def test_gc(isolated_dir):
    ...
    create_des(f)
    f.write("basic_nitf.ntf")
    f2 = NitfFile("basic_nitf.ntf")  # noqa: F841
...
def test_tre_overflow_write_security(isolated_dir, security_fake):
    """Repeat of test_basic_write, but also include two really big TREs that
