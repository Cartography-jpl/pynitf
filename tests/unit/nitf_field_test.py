...
def test_float_to_fixed_width(evil_float1, evil_float2, evil_float3):
...
def test_nitf_field_frmt2():
    ...
    f = NitfField(None, "foo", 12, float, None, {"frmt": _tre_rpc_coeff_format})
    f[()] = 1.23
    assert f[()] == 1.23
...
def test_nitf_field_hardcoded_value():
    ...
    f = NitfField(None, "foo", 4, int, None, {"default": 20, "hardcoded_value": True})
    assert f[()] == 20
    assert f.bytes() == b"0020"
...
def test_nitf_field_optional_char():
    ...
    with pytest.raises(RuntimeError):
        f[()] = None
    f = NitfField(None, "foo", 4, int, None, {"optional": True, "optional_char": "-"})
    f[()] = None
    assert f[()] is None
    assert f.bytes() == b"----"
    fh = io.BytesIO()
    f.write_to_file(fh, ())
    assert fh.getvalue() == b"----"
    f2 = NitfField(None, "foo", 4, int, None, {"optional": True, "optional_char": "-"})
    fh2 = io.BytesIO(fh.getvalue())
    f2.read_from_file(fh2, False, ())
    assert f2[()] is None
    fh2 = io.BytesIO(fh.getvalue())
...
def test_string_field_data():
    f = StringFieldData(
        ...
        None,
        None,
        {"field_value_class": StringFieldData, "size_not_updated": True},
    )
    f[()] = "blahblahblah"
...
# pickling working
@pytest.mark.skip(reason="don't want to assume dill is available")
...
def test_loop(nitf_diff_field_struct):
    ...
    class TestFieldStruct(FieldStruct):
        desc = [
            ["fhdr", "", 4, str, {"default": "NITF"}],
            ["numi", "", 3, int],
            [["loop", "f.numi"], ["lish", "", 6, int], ["li", "", 10, int]],
        ]
...
"""
    ...
    assert (
        fh.getvalue()
        == b"NITF0040000010000000005000002000000000600000300000000070000040000000008"
    )
    t2 = copy.deepcopy(t)
    assert d.compare_obj(t, t2)
    t2.li[1] = 17
    assert not d.compare_obj(t, t2)
...
def test_nested_loop(nitf_diff_field_struct):
    ...
    assert (
        fh.getvalue()
        == b"NITF0020000000030000000000000000001000000000000000000040000000000000000000000000000000000000020"
    )
    fh2 = io.BytesIO(fh.getvalue())
    t2 = TestFieldStruct()
    t2.read_from_file(fh2)
...
def test_nested_loop2(nitf_diff_field_struct):
    ...
    assert (
        fh.getvalue()
        == b""
    )
    fh2 = io.BytesIO(fh.getvalue())
    t2 = TestFieldStruct()
    t2.read_from_file(fh2)
...
def test_nested_loop3(nitf_diff_field_struct):
    ...
    assert (
        fh.getvalue()
        == b""
    )
    fh2 = io.BytesIO(fh.getvalue())
    t2 = TestFieldStruct()
    t2.read_from_file(fh2)
