...
class NitfDesFieldStruct(NitfDes, FieldStruct):
    ...
    def read_from_file(self, fh, seg_index=None, force_raw_read=False):
        ...
        if self.data_after_tre_size != 0:
            if not self.data_after_allowed:
                raise RuntimeError(
                    "DES %s TRE length was expected to be %d but was actually %d"
                    % (self.des_tag, self._seg().data_size, (self.data_start - t))
                )
            if self.data_copy:
    ...
    def __str__(self):
        ...
        if not self.des_implementation_field or (
            self._seg and self._seg().nitf_file and self._seg().nitf_file.report_raw
        ):
            self.update_raw_field()
            res = io.StringIO()
            self.str_hook(res)
            ...
            if self.data_after_tre_size and self.data_after_tre_size != 0:
                print("Extra data of length %d" % self.data_after_tre_size, file=res)
            return res.getvalue()
        res = io.StringIO()
        self.str_hook(res)
        if self.user_subheader:
...
class DesFieldStructDiff(FieldStructDiff):
    ...
    def handle_diff(self, des1, des2, nitf_diff):
        if not isinstance(des1, NitfDesFieldStruct) or not isinstance(
            des2, NitfDesFieldStruct
        ):
            return (False, None)
...
class NitfDesCopy(NitfDes):
    ...
    def write_to_file(self, fh):
        ...
        if self.data is None:
            raise RuntimeError(
                "Can only write data after we have read it in NitdDesCopy"
            )
        fh.write(self.data)
