...
class NitfData(object, metaclass=abc.ABCMeta):
    ...
    def __init__(self, seg=None):
        ...
        if seg:
            ...
            self._seg = None
            self._shared_header = NitfSharedHeader(self.sh_class, self.uh_class)
...
class DataPlaceHolderDiff(NitfDiffHandle):
    ...
    def handle_diff(self, g1, g2, nitf_diff):
        if not isinstance(g1, NitfDataPlaceHolder) or not isinstance(
            g2, NitfDataPlaceHolder
        ):
            return (False, None)
...
NitfSegmentDataHandleSet.add_default_handle(NitfDataPlaceHolder, priority_order=-9999)
