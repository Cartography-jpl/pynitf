...
class TreENGRDA(Tre, MutableMapping):
    ...
    def summary(self):
        res = io.StringIO()
        print("TRE - ENGRDA %s: %d Entries:" % (self.resrc, self.recnt), file=res)
        return res.getvalue()
    ...
    def __setitem__(self, key, v):
        if len(v) != 2:
            raise RuntimeError(
                "Need to give two value to set in ENGRDA, the data and the units"
            )
...
def _engrda_as_hash(self):
    ...
    for t in self.tre_list:
        if t.tre_tag == "ENGRDA":
            if t.resrc in res:
                raise RuntimeError(
                    "Two ENGRDA TREs have the same resrc value of '%s'" % t.resrc
                )
            res[t.resrc] = t
    return res
