...
class NitfFileHandle(NitfDiffHandle):
    ...
    def handle_diff(self, f1, f2, nitf_diff):
        if not isinstance(f1, NitfFile) or not isinstance(f2, NitfFile):
            return (False, None)
        is_same = True
        is_same = nitf_diff.compare_obj(f1.file_header, f2.file_header) and is_same
        for desc, lis1, lis2 in [
            ...
            if len(lis1) != len(lis2):
                logger = logging.getLogger("nitf_diff")
                logger.difference(
                    "File 1 has %d %s while file 2 has %d"
...
class SegmentDiff(NitfDiffHandle):
    ...
    def handle_diff(self, seg1, seg2, nitf_diff):
        if not isinstance(seg1, NitfSegment) or not isinstance(seg2, NitfSegment):
            return (False, None)
        if seg1.segment_type() != seg2.segment_type():
            return (False, None)
        if (
            seg1.subheader
            and hasattr(seg1.subheader, "desid")
            and seg1.subheader.desid == "TRE_OVERFLOW"
            and seg2.subheader
            and hasattr(seg2.subheader, "desid")
            and seg2.subheader.desid == "TRE_OVERFLOW"
        ):
        ...
        with nitf_diff.diff_context(seg1.short_desc()):
            ...
            if seg1.user_subheader:
                is_same = (
                    nitf_diff.compare_obj(seg1.user_subheader, seg2.user_subheader)
                    and is_same
                )
            ...
            if len(t1) != len(t2):
                logger.difference(
                    "Segment 1 has %d TREs while Segment 2 has %d", len(t1), len(t2)
                )
                is_same = False
            for i in range(min(len(t1), len(t2))):
                is_same = nitf_diff.compare_obj(t1[i], t2[i]) and is_same
