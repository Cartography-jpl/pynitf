...
desc = [
    ...
    [
        "interp_order_att",
        "Order of Lagrange Interpolation Polynomials",
        1,
        int,
        {"condition": "f.interp_type_att==2"},
    ],
    ["att_type", "Attitude Type", 1, int],
    ["eci_ecf_att", "Coordinate Reference Frame Flag", 1, int],
    [
        "dt_att",
        "Time interval between attitude reference points",
        13,
        float,
        {"frmt": "%013.9lf"},
    ],
    ...
    [
        "t0_att",
        "UTC Timestamp of First Attitude Reference Point",
        16,
        float,
        {"frmt": "%016.9lf"},
    ],
    ...
    [
        ...
        [
            "q1",
            "Quaternion Q1 of Attitude Reference Point",
            18,
            float,
            {"frmt": _quat_format},
        ],
        [
            "q2",
            "Quaternion Q2 of Attitude Reference Point",
            18,
            float,
            {"frmt": _quat_format},
        ],
        [
            "q3",
            "Quaternion Q3 of Attitude Reference Point",
            18,
            float,
            {"frmt": _quat_format},
        ],
        [
            "q4",
            "Quaternion Q4 of Attitude Reference Point",
            18,
            float,
            {"frmt": _quat_format},
        ],
...
desid_to_user_subheader_handle.add_des_user_subheader(
    "CSATTB", DesAssociatedUserSubheader
)
...
class CsattbDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        with nitf_diff.diff_context("DesCSATTB"):
            if not isinstance(h1, DesCSATTB) or not isinstance(h2, DesCSATTB):
                return (False, None)
