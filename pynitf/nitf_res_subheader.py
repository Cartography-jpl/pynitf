...
desc = [
    [
        "re",
        "Reserve Extension Subheader Identifier",
        2,
        str,
    ...
    [
        "reclas",
        "Reserve Extension Segment Security Classification",
        1,
        str,
        {"default": "U"},
    ],
    ...
    ["resshf", "", "f.resshl", None, {"field_value_class": BytesFieldData}],
]
...
class ResSubheaderDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        with nitf_diff.diff_context("Subheader", add_text=True):
            if not isinstance(h1, NitfResSubheader) or not isinstance(
                h2, NitfResSubheader
            ):
                return (False, None)
