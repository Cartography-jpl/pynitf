...
class ImagePlaceHolderDiff(NitfDiffHandle):
    def handle_diff(self, d1, d2, nitf_diff):
        if not isinstance(d1, NitfImagePlaceHolder) or not isinstance(
            d2, NitfImagePlaceHolder
        ):
            return (False, None)
        logger.warn(
            "Skipping image '%s', don't know how to read it.", d1.subheader.iid1
        )
        return (True, True)
...
class NitfImageReadNumpy(NitfImageWithSubset):
    ...
    def __str__(self):
        if self.shape[0] == 1:
            return "NitfImageReadNumpy %d x %d %s image" % (
                self.shape[1],
                self.shape[2],
                str(self.dtype.newbyteorder("=")),
            )
        return "NitfImageReadNumpy %d x %d x %d %s image" % (
            self.shape[0],
            self.shape[1],
            self.shape[2],
            str(self.dtype.newbyteorder("=")),
    ...
    def read_from_file(self, fh, segindex=None):
        ...
        if (
            ih.imode == "P"
            and ih.nbpr == 1
            and ih.nbpc == ih.shape[1]
            and ih.nppbh == ih.shape[2]
        ):
        ...
        if self.do_mmap:
            ...
            self.data = np.ndarray(
                self.shape, dtype=ih.dtype, buffer=self.mm, strides=strides, offset=foff
            )
            fh.seek(self.data.size * self.data.itemsize + foff, 0)
        else:
            self.data = np.fromfile(
                fh, dtype=ih.dtype, count=ih.nrows * ih.ncols * ih.number_band
            )
            if strides is not None:
                self.data = np.lib.stride_tricks.as_strided(
                    self.data, shape=self.shape, strides=strides
                )
...
class ImageWithSubsetDiff(NitfDiffHandle):
    def handle_diff(self, d1, d2, nitf_diff):
        if not isinstance(d1, NitfImageWithSubset) or not isinstance(
            d2, NitfImageWithSubset
        ):
            return (False, None)
        is_same = True
        t1 = d1[:, :, :]
...
class NitfImageWriteNumpy(NitfImageWriteDataOnDemand):
    ...
    def __str__(self):
        if self.shape[0] == 1:
            return "NitfImageWriteNumpy %d x %d %s image" % (
                self.shape[1],
                self.shape[2],
                str(self.dtype.newbyteorder("=")),
            )
        return "NitfImageWriteNumpy %d x %d x %d %s image" % (
            self.shape[0],
            self.shape[1],
            self.shape[2],
            str(self.dtype.newbyteorder("=")),
    ...
    def data_to_write(self, d, bstart, lstart, sstart):
        d[:, :, :] = self._data[
            bstart : (bstart + d.shape[0]),
            lstart : (lstart + d.shape[1]),
            sstart : (sstart + d.shape[2]),
        ]
...
class NitfRadCalc(object):
    ...
    def data_to_write(self, d, bstart, lstart, sstart):
        ...
        d[:, :, :] = (
            self.dn[bs, ls, ss] * self.gain[bs, ls, ss] + self.offset[bs, ls, ss]
        )


NitfSegmentDataHandleSet.add_default_handle(NitfImageReadNumpy)
NitfSegmentDataHandleSet.add_default_handle(NitfImagePlaceHolder, priority_order=-1000)

__all__ = [
    "NitfImageWithSubset",
    "NitfImagePlaceHolder",
    "NitfImageReadNumpy",
    "NitfImageWriteDataOnDemand",
    "NitfImageWriteNumpy",
    "NitfRadCalc",
]
