...
class NitfSegmentJson(NitfSegment):
    ...
    def notify_segment_merge(self, seg_list):
        ...
        if self.primary_key() is not None:
            slist = [s for s in seg_list if s.primary_key() == self.primary_key()]
        if len(slist) != 1:
    ...
    def __getstate__(self):
        return {
            "shared_header": self._shared_header,
...
class NitfFileJson:
    ...
    def create_delta(cls, fin1, fin2):
        ...
        for tseg in fin1.text_segment:
            f.text_segment.append(
                NitfTextSegmentJson.create_delta(tseg, include_data=True)
            )
        for dseg in fin1.des_segment:
            ...
            if dseg.subheader.desid != "TRE_OVERFLOW":
                ...
                f.des_segment.append(
                    NitfDesSegmentJson.create_delta(dseg, include_data=include_data)
                )
        return f
    ...
    def notify_file_merge(self, file_list, fparent):
        ...
        if cnt > 1:
            raise RuntimeError(
                "We don't currently handle merging with more than one file. Could probably do this, but would need to work out the logic for this. Right now we treat this as an error."
            )
    ...
    def read(self, file_name):
        ...
        (
            ...
            self.res_segment,
            self.tre_list,
        ) = jsonpickle.decode(open(file_name).read())
