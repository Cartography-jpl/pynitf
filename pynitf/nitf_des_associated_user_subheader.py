...
desc = [
    ...
    [
        "reservedsubh",
        "Reserved for Future Additions to the DES User-Defined Subheader",
        "f.reservedsubh_len",
        None,
...
class DesAssociatedUserSubheaderDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        with nitf_diff.diff_context("DesAssociatedUserSubheader"):
            if not isinstance(h1, DesAssociatedUserSubheader) or not isinstance(
                h2, DesAssociatedUserSubheader
            ):
                return (False, None)
