...
class Tre(FieldStruct):
    ...
    def read_from_tre_bytes(self, bt, nitf_literal=False):
        if self.tre_implementation_field:
            ...
            setattr(
                self,
                self.tre_implementation_field,
                self.tre_implementation_class.read_tre_string(t),
            )
    ...
    def read_from_file(self, fh, delayed_read=False):
        ...
        if self.tre_implementation_field:
            ...
            if sz != cel:
                raise RuntimeError(
                    "TRE length was expected to be %d but was actually %d" % (cel, sz)
                )
...
def prepare_tre_write(tre_list, header, des_list, field_list=[], seg_index=0):
...
class TreDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        if not isinstance(h1, Tre) or not isinstance(h2, Tre):
            return (False, None)
        if h1.tre_tag != h2.tre_tag:
            logger.difference(
                "TREs tags don't match. TRE 1 '%s' and TRE 2 '%s'",
                h1.tre_tag,
                h2.tre_tag,
            )
            return (True, False)
...
class TreUnknownDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        if not isinstance(h1, TreUnknown) or not isinstance(h2, TreUnknown):
            return (False, None)
        if h1.tre_tag != h2.tre_tag:
            logger.difference(
                "TREs tags don't match. TRE 1 '%s' and TRE 2 '%s'",
        ...
        if h1.tre_bytes != h2.tre_bytes:
            logger.difference(
                "TREs bytes don't match. TRE 1 '%s' and TRE 2 '%s'",
