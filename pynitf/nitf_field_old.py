...
class StringFieldDataOld(FieldDataOld):
    def get_print(self, parent_obj,key):
        t = self.get(parent_obj,key)
        if(len(t) == 0):
            return "Not used"
...
class NumFieldDataOld(FieldDataOld):
    def __init__(self, field_name, size_field, ty, loop, options):
...
class IntFieldDataOld(NumFieldDataOld):
    def __init__(self, field_name, size_field, ty, loop, options):
        self.print_format = '%d'
...
class FloatFieldDataOld(NumFieldDataOld):
    def __init__(self, field_name, size_field, ty, loop, options):
        self.format = '>f'
        super().__init__(field_name, size_field, ty, loop, options)
...
class FieldStructDiffOld(NitfDiffHandle):
