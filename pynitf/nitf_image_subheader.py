...
desc = [
    ["im", "File Part Type", 2, str, {"default": "IM", "hardcoded_value": True}],
    ...
    [["loop", "f.nicom"], ["icom", "Image Comments", 80, str]],
    ...
    [
        ...
        [
            "ifc",
            "Band Filter Condition",
            1,
            str,
            {"default": "N", "hardcoded_value": True},
        ],
        [
            "imflt",
        ...
        [
            ["loop", "f.nluts[i1]"],
            ["lutd", "", "f.nelut[i1]", None, {"field_value_class": BytesFieldData}],
        ],
    ],
    ["isync", "Image Sync Code", 1, int, {"default": 0, "hardcoded_value": True}],
    ...
    [
        ...
        "f.udidl",
        None,
        {"field_value_class": BytesFieldData, "size_offset": 3},
    ],
...
class NitfImageSubheader(FieldStruct):
    ...
    def dtype(self):
        ...
        if self.nbpp == 8 and self.pvtype == "INT":
            ...
            raise RuntimeError(
                "Unrecognized nbpp %d and pvtype %s" % (self.nbpp, self.pvtype)
            )
    ...
    def geolo_corner(self):
        ...
        icoords = self.icords
        corners, utm_zone = nitf_read_igeolo(self.icords, self.igeolo.encode("utf-8"))
...
def check_igeolo_utm_x(name, x):
    ...
    if t < -100000 or t >= 1000000:
        raise RuntimeError(
            "Attempt to write UTM easting %s=%d which is outside the valid range."
            % (name, t)
        )
...
def check_igeolo_utm_y(name, y):
    ...
    if t < -1000000 or t >= 10000000:
        raise RuntimeError(
            "Attempt to write UTM northing %s=%d which is outside the valid range."
            % (name, t)
        )
...
def nitf_write_igeolo(chICORDS, nZone, df):
    ...
    if chICORDS == "G":
        if (
            math.fabs(dfULX) > 180
            or math.fabs(dfURX) > 180
            or math.fabs(dfLRX) > 180
            or math.fabs(dfLLX) > 180
            or math.fabs(dfULY) > 90
            or math.fabs(dfURY) > 90
            or math.fabs(dfLRY) > 90
            or math.fabs(dfLLY) > 90
        ):
            raise RuntimeError(
    ...
    if chICORDS in ("N", "S"):
        ...
        szIGEOLO[0:15] = (
            "%02d%06d%07d" % (nZone, int(math.floor(dfULY)), int(math.floor(dfULX)))
        ).encode("utf-8")
        szIGEOLO[15:30] = (
            "%02d%06d%07d" % (nZone, int(math.floor(dfURY)), int(math.floor(dfURX)))
        ).encode("utf-8")
        szIGEOLO[30:45] = (
            "%02d%06d%07d" % (nZone, int(math.floor(dfLRY)), int(math.floor(dfLRX)))
        ).encode("utf-8")
        szIGEOLO[45:60] = (
            "%02d%06d%07d" % (nZone, int(math.floor(dfLLY)), int(math.floor(dfLLX)))
        ).encode("utf-8")
    return szIGEOLO
...
def nitf_read_igeolo(chICORDS, s):
    ...
    for pszCoordPair in (s[0:15], s[15:30], s[30:45], s[45:60]):
        if chICORDS in ("N", "S"):
            ...
            res.append([float(pszCoordPair[2:8]), float(pszCoordPair[8:15])])
...
def set_default_image_subheader(
    ih,
    nrow,
    ncol,
    data_type,
...
class ImageSubheaderDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        with nitf_diff.diff_context("Subheader", add_text=True):
            if not isinstance(h1, NitfImageSubheader) or not isinstance(
                h2, NitfImageSubheader
            ):
                return (False, None)
...
_default_config["exclude"] = ["udidl", "udofl", "udid", "ixshdl", "ixofl", "ixshd"]


NitfDiffHandleSet.default_config["Image Subheader"] = _default_config


__all__ = ["NitfImageSubheader", "set_default_image_subheader", "ImageSubheaderDiff"]
