...
class NitfFile(object):
    ...
    def __init__(self, file_name=None, security=security_unclassified):
        ...
        self.user_subheader_handle_set = copy.copy(
            NitfSegmentUserSubheaderHandleSet.default_handle_set()
        )
    ...
    def __str__(self):
        ...
        for arr, name in [
            ...
            for i, seg in enumerate(arr):
                print("%s segment %d of %d" % (name, i + 1, len(arr)), file=res)
    ...
    def read(self, file_name):
        ...
        with open(file_name, "rb") as fh:
            ...
            if self.file_header.fl == 999999999999:
                raise RuntimeError(
                    "We don't currently support reading streaming NITF files"
                )
            self.image_segment = [
                NitfImageSegment(
                    header_size=self.file_header.lish[i],
            ...
            self.graphic_segment = [
                NitfGraphicSegment(
                    header_size=self.file_header.lssh[i],
            ...
            self.text_segment = [
                NitfTextSegment(
                    header_size=self.file_header.ltsh[i],
            ...
            self.des_segment = [
                NitfDesSegment(
                    header_size=self.file_header.ldsh[i],
            ...
            self.res_segment = [
                NitfResSegment(
                    header_size=self.file_header.lresh[i],
            ...
            self.tre_list = read_tre(
                self.file_header,
                self.des_segment,
                [["xhdl", "xhdlofl", "xhd"], ["udhdl", "udhofl", "udhd"]],
            )
    ...
    def write(self, file_name):
        ...
        self.des_segment = ListNitfFileReference(
            self,
            [
                dseg
                for dseg in self.des_segment
                if (dseg.subheader.desid.encode("utf-8") != b"TRE_OVERFLOW")
            ],
        )
