...
desc = [
    ["content_headers_len", "Length in bytes of the CONTENT_HEADERS field", 4, int],
    [
        "content_headers",
        "Metadata describing the embedded content",
        "f.content_headers_len",
        None,
...
class DesExtContentHeaderDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        with nitf_diff.diff_context("DesExtContentHeader"):
            if not isinstance(h1, DesExtContentHeader) or not isinstance(
...
class DesEXT_DEF_CONTENT(NitfDes):
    ...
    def read_from_file(self, fh, seg_index=None):
        ...
        foff = fh.tell()
        self.data = np.memmap(
            fh, mode="r", dtype=np.int8, shape=(self.data_size,), offset=foff
        )
...
desid_to_user_subheader_handle.add_des_user_subheader(
    "EXT_DEF_CONTENT", DesExtContentHeader
)
...
class DesExtDefContentDiff(NitfDiffHandle):
    ...
    def handle_diff(self, d1, d2, nitf_diff):
        with nitf_diff.diff_context("DesExtDefContent"):
            if not isinstance(d1, DesEXT_DEF_CONTENT) or not isinstance(
...
class DesEXT_h5(DesEXT_DEF_CONTENT):
    ...
    def attach_file(self, file):
        ...
        self.file = file
        t = 'attachment; filename="%s"; creation-date="%s"' % (
            os.path.basename(file),
            datetime.datetime.fromtimestamp(os.stat(file).st_ctime),
        )
...
NitfSegmentDataHandleSet.add_default_handle(DesEXT_DEF_CONTENT, priority_order=-1)
