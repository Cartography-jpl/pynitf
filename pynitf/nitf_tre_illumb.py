...
class TreILLUMB(Tre):
    ...
    def summary(self):
        ...
        print("Number of Other Natural Light Sources: %d" %
              (self.num_others), file=res)
        for i in range(self.num_others):
            print("Other Light Source %d Name: %s" %
                  (i, self.other_name[i]), file=res)
        ...
        print("Vertical Datum Reference: %s" % (self.vertical_datum_ref),
              file=res)
        print("Code (Category) of Vertical Reference: %s" %
              (self.vertical_ref_code), file=res)
        print("Bit-wise Existence Mask Fields:", file=res)
        print("%5s b23 RAD_QUANTITY, RADQ_UNIT" %
              (str(self.existence_mask & 0x800000)), file=res)
        print("%5s b22 SUN_AZIMUTH, SUN_ELEV" %
              (str(self.existence_mask & 0x400000)), file=res)
        print("%5s b21 MOON_AZIMUTH, MOON_ELEV" %
              (str(self.existence_mask & 0x200000)), file=res)
        print("%5s b20 MOON_PHASE_ANGLE" %
              (str(self.existence_mask & 0x100000)), file=res)
        print("%5s b19 MOON_ILLUM_PERCENT" %
              (str(self.existence_mask & 0x80000)), file=res)
        print("%5s b18 OTHER_AZIMUTH, OTHER_ELEV" %
              (str(self.existence_mask & 0x40000)), file=res)
        print("%5s b17 SENSOR_AZIMUTH, SENSOR_ELEV" %
              (str(self.existence_mask & 0x20000)), file=res)
        print("%5s b16 CATS_ANGLE" % (str(self.existence_mask & 0x10000)),
              file=res)
        print("%5s b15 SUN_GLINT_LAT, SUN_GLINT_LON" %
              (str(self.existence_mask & 0x8000)), file=res)
        print("%5s b14 CATM_ANGLE" % (str(self.existence_mask & 0x4000)),
              file=res)
        print("%5s b13 MOON_GLINT_LAT, MOON_GLINT_LON" %
              (str(self.existence_mask & 0x2000)), file=res)
        print("%5s b12 SUN_ILLUM_METHOD, SUN_ILLUM" %
              (str(self.existence_mask & 0x1000)), file=res)
        print("%5s b11 MOON_ILLUM_METHOD, MOON_ILLUM" %
              (str(self.existence_mask & 0x800)), file=res)
        print("%5s b10 SOL_LUN_DIST_ADJUST, TOT_SUNMOON_ILLUM" %
              (str(self.existence_mask & 0x400)), file=res)
        print("%5s b9 OTHER_ILLUM_METHOD, OTHER_ILLUM" %
              (str(self.existence_mask & 0x200)), file=res)
        print("%5s b8 ART_ILLUM_METHOD, ART_ILLUM_MIN, ART_ILLUM_MAX" %
              (str(self.existence_mask & 0x100)), file=res)
        ...
        print("Number of Illumination Conditions: %d" % (self.num_illum_sets),
              file=res)
        for i in range(self.num_illum_sets):
            print("Illumination Condition Date and Time: %s" %
                  (self.datetime[i]), file=res)
            ...
            if self.existence_mask & 0x400000:
                print("Sun Azimuth Angle: %05.1f" %
                      (self.sun_azimuth[i]), file=res)
                print("Sun Elevation Angle: %05.1f" %
                      (self.sun_elev[i]), file=res)
            if self.existence_mask & 0x200000:
                print("Lunar Azimuth Angle: %05.1f" %
                      (self.moon_azimuth[i]), file=res)
                print("Lunar Elevation Angle: %05.1f" %
                      (self.moon_elev[i]), file=res)
            if self.existence_mask & 0x100000:
                print("Phase Angle of the Moon in Degrees: %+06.1f" %
                      (self.moon_phase_angle[i]), file=res)
            if self.existence_mask & 0x80000:
                print("Lunar Illumination Percentage: %03d" %
                      (self.moon_illum_percent[i]), file=res)
            if self.existence_mask & 0x40000:
                for j in range(self.num_others):
                    print("Other Natural Light Source Azimuth Angle: %05.1f" %
                          (self.other_azimuth[i,j]), file=res)
                    print("Other Natural Light Source Elevation Angle: %05.1f"
                          % (self.other_elev[i,j]), file=res)
            if self.existence_mask & 0x20000:
                print("Sensor Azimuth Angle: %05.1f" %
                      (self.sensor_azimuth[i]), file=res)
                print("Sensor Elevation Angle: %05.1f" %
                      (self.sensor_elev[i]), file=res)
            if self.existence_mask & 0x10000:
                print("Camera-Target-Sun Angle: %05.1f" %
                      (self.cats_angle[i]), file=res)
            if self.existence_mask & 0x8000:
                print("Sun Glint Latitude: %+010.6f" %
                      (self.sun_glint_lat[i]), file=res)
                print("Sun Glint Longitude: %+011.6f" %
                      (self.sun_glint_lon[i]), file=res)
            if self.existence_mask & 0x4000:
                print("Camera-Target-Moon Angle: %05.1f" %
                      (self.catm_angle[i]), file=res)
            if self.existence_mask & 0x2000:
                print("Moon Glint Latitude: %+010.6f" %
                      (self.moon_glint_lat[i]), file=res)
                print("Moon Glint Longitude: %+011.6f" %
                      (self.moon_glint_lon[i]), file=res)
            if self.existence_mask & 0x400:
                print("Solar/Lunar Distance Adjustment: %7.5f" %
                      (self.sol_lun_dist_adjust[i]), file=res)
            for j in range(self.num_bands):
                if self.existence_mask & 0x1000:
                    print("Solar Illumination Estimation Method %s" %
                          (self.sun_illum_method[i,j]), file=res)
                    print("Solar Illumination: %16E" %
                          (self.sun_illum[i,j]), file=res)
                if self.existence_mask & 0x800:
                    print("Lunar Illumination Estimation Method: %s" %
                          (self.moon_illum_method[i,j]), file=res)
                    print("Lunar Illumination: %16E" %
                          (self.moon_illum[i,j]), file=res)
                if self.existence_mask & 0x400:
                    print("Total Illumination from Sun and Moon: %16E" %
                          (self.tot_sunmoon_illum[i,j]), file=res)
                if self.existence_mask & 0x200:
                    for k in range(self.num_others):
                        print("Other Natural Light Source Illumination Estimation Method: %s" %
                              (self.other_illum_method[i,j,k]), file=res)
                        print("Other Natural Light Source Illumination: %16E"
                              % (self.other_illum[i,j,k]), file=res)
                if self.existence_mask & 0x100:
                    print("Artificial Illumination Estimation Method: %s"
                          % (self.art_illum_method[i,j]), file=res)
                    print("Minimum Artificial Illumination: %16E"
                          % (self.art_illum_min[i,j]), file=res)
                    print("Maximum Artificial Illumination: %16E"
                          % (self.art_illum_max[i,j]), file=res)
