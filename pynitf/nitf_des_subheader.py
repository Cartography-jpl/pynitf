...
desc = [
    [
        "de",
        "Data Extension Subheader Identifier",
        2,
        str,
    ...
    [
        "desclas",
        "Data Extension Segment Security Classification",
        1,
        str,
        {"default": "U"},
    ],
    ...
    ["desshf", "", "f.desshl", None, {"field_value_class": BytesFieldData}],
]
...
class DesSubheaderDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        with nitf_diff.diff_context("Subheader", add_text=True):
            if not isinstance(h1, NitfDesSubheader) or not isinstance(
                h2, NitfDesSubheader
            ):
                return (False, None)
            if h1.desid == "TRE_OVERFLOW" and h2.desid == "TRE_OVERFLOW":
...
_default_config["exclude"] = ["desoflw", "desitem", "desshl", "desshf"]
