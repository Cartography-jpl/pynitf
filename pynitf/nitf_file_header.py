...
"""
desc = [
    ["fhdr", "File Profile Name", 4, str, {"default": "NITF", "hardcoded_value": True}],
    ["fver", "File Version", 5, str, {"default": "02.10", "hardcoded_value": True}],
    ["clevel", "Complexity Level", 2, int, {"default": 3}],
    ["stype", "Standard Type", 4, str, {"default": "BF01", "hardcoded_value": True}],
    ...
    ["nums", "", 3, int],
    [["loop", "f.nums"], ["lssh", "", 4, int], ["ls", "", 6, int]],
    ["numx", "", 3, int],
    ["numt", "", 3, int],
    [["loop", "f.numt"], ["ltsh", "", 4, int], ["lt", "", 5, int]],
    ["numdes", "", 3, int],
    [["loop", "f.numdes"], ["ldsh", "", 4, int], ["ld", "", 9, int]],
    ["numres", "", 3, int],
    [["loop", "f.numres"], ["lresh", "", 4, int], ["lre", "", 7, int]],
    ["udhdl", "", 5, int],
...
class NitfFileHeader(FieldStruct):
    ...
    def summary(self):
        ...
        print(
            "%d Image Segments, %d Graphic Segments, %d Text Segments, %d DESs"
            % (self.numi, self.nums, self.numt, self.numdes),
            file=res,
        )
        return res.getvalue()
...
class FileHeaderDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        with nitf_diff.diff_context("NITF File Header"):
            if not isinstance(h1, NitfFileHeader) or not isinstance(h2, NitfFileHeader):
                return (False, None)
