...
desc = [
    ...
    [
        "pq_bit_value",
        "Pixel Quality Bit Value",
        1,
        str,
        {"default": "1", "hardcoded_value": True},
    ],
    [["loop", "f.npixqual"], ["pq_condition", "Pixel Quality Condition", 40, str]],
]
...
class TrePIXQLA(Tre):
    ...
    def summary(self):
        res = io.StringIO()
        print(
            "TRE - PIXQLA %s Associated ISs, %d Quality Conditions"
            % (self.numais, self.npixqual),
            file=res,
        )
        return res.getvalue()
