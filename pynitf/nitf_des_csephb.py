...
desc = [
    ...
    [
        "t0_ephem",
        "UTC Timestamp of First Ephemeris Vector",
        16,
        float,
        {"frmt": "%016.9lf"},
    ],
...
desid_to_user_subheader_handle.add_des_user_subheader(
    "CSEPHB", DesAssociatedUserSubheader
)
...
class CsephbDiff(FieldStructDiff):
    ...
    def handle_diff(self, h1, h2, nitf_diff):
        with nitf_diff.diff_context("DesCSEPHB"):
            if not isinstance(h1, DesCSEPHB) or not isinstance(h2, DesCSEPHB):
                return (False, None)
