...
class NitfImageHDF5(NitfImage):
    ...
    def __str__(self):
        return "NitfImageHFD5 %d x %d, %d bands, blob size %d" % (
            self.image_subheader.nrows,
            self.image_subheader.ncols,
            self.image_subheader.nbands,
            self.data_size,
        )
    ...
    def writePartialRows(self, fh, data, v_low, v_high, h_low, h_high):
        ...
        fillData = np.ndarray(
            shape=(npixels, self.bands()),
            buffer=np.array([0] * (npixels * self.bands()), data.dtype),
            dtype=data.dtype,
        )
    ...
    def writeEmptyRows(self, fh, data, h_high):
        ...
        fillData = np.ndarray(
            shape=(h_high - ih.nrows, ih.nppbv, self.bands()),
            buffer=np.array(
                [0] * ((h_high - ih.nrows) * ih.nppbv * self.bands()), data.dtype
            ),
            dtype=data.dtype,
        )
        print(fillData.shape)
        ...
        for i in range(ih.nbpr):
            for j in range(ih.nbpc):
                ...
                if h_high > ih.nrows and v_high > ih.ncols:
                    print(
                        "%d is larger than %d, horizontally and \
                ...
                # ------------
                    ...
                    print(
                        "we're at exact match on both dimensions along block boundaries"
                    )
