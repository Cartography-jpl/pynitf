...
class DiffHandle(object, metaclass=abc.ABCMeta):
    ...
    def process_field_value_list(self, type, list1, parent1, list2, parent2):
            ...
            if not isinstance(field, _FieldLoopStruct):
                if field.field_name is not None:
                    ...
                    if hasattr(field, "eq_fun") and field.eq_fun != None:
                        ...
                        this_is_same = field.eq_fun[0](val1, val2, *field.eq_fun[1:])
                    else:
                        val1 = field.value(parent1)
                        val2 = list2[index].value(parent2)
                    ...
                    if not this_is_same:
                        ...
                        self.logger.error(
                            "%s 1 has field %s as %s while %s 2 has %s"
                            % (type, field.field_name, val1, type, val2)
                        )
                        is_same = False
            else:
                is_same = (
                    self.process_field_value_list(
                        type,
        ...
        self.logger.debug(
            "process_field_value_list(%s) returning>>> %s" % (type, is_same)
        )
        return is_same

    def compare_tres(self, tre1, tre2):
        return self.process_field_value_list(
            "TRE", tre1.field_value_list, tre1, tre2.field_value_list, tre2
        )
    ...
    def handle_diff(self, obj1, obj2, index=-1):
        ...
        try:
            if isinstance(obj1, TreUnknown) and isinstance(obj2, TreUnknown):
                ...
                is_same = self.process_field_value_list(
                    self.obj_type,
                    obj1.field_value_list,
                    obj1,
                    obj2.field_value_list,
                    obj2,
                )
        ...
        self.logger.debug(
            "TREFileHeadHandle(%s) returning>>> %s" % (self.obj_type, is_same)
        )
        return (True, is_same)
...
class ISegHandle(DiffHandle):
    ...
    def handle_diff(self, obj1, obj2, index=-1):
        ...
        if isinstance(obj1.data, NitfImagePlaceHolder) or isinstance(
            obj2.data, NitfImagePlaceHolder
        ):
            ...
            if obj1.data.shape == obj2.data.shape:
                ...
                if (
                    ...
                    and "tolerances" in self.config
                    and "method" in self.config["tolerances"][index]
                ):
                    tolerance_config = self.config["tolerances"][index]

                    if (
                        tolerance_config["method"] == "histogram"
                        and "numBins" in tolerance_config
                    ):
                        min1 = 0
                        min2 = 0
                        max1 = 0
                        ...
                        if "percent" in tolerance_config:
                            is_same = is_same and np.allclose(
                                hist1, hist2, rtol=tolerance_config["percent"] / 100
                            )
                        elif "value" in tolerance_config:
                            is_same = is_same and np.allclose(
                                hist1, hist2, rtol=tolerance_config["value"]
                            )
                        ...
                        if "percent" in tolerance_config:
                            is_same = is_same and (
                                count < (total * (tolerance_config["percent"] / 100))
                            )
                        elif "value" in tolerance_config:
                            is_same = is_same and (
                                count < (total * tolerance_config["value"])
                            )
...
def _handle_type(lis1, lis2, nm, handler, logger):
    ...
    if len(lis1) != len(lis2):
        logger.error(
            "File 1 has %d file level %s while file 2 has %d"
    ...
    for i in range(min(len(lis1), len(lis2))):
        ...
        if hasattr(lis1[i], "tre_list"):
            if len(lis1[i].tre_list) != len(lis2[i].tre_list):
                logger.error(
                    "File 1 has %d %s level TREs while file 2 has %d"
                    % (len(lis1[i].tre_list), nm, len(lis2[i].tre_list))
                )
                is_same = False
            logger.debug(
                "Comparing %d tres of %s index %d"
                % (min(len(lis1[i].tre_list), len(lis2[i].tre_list)), nm, i)
            )
            for j in range(min(len(lis1[i].tre_list), len(lis2[i].tre_list))):
                status = tre_handle.handle_diff(
                    lis1[i].tre_list[j], lis2[i].tre_list[j]
                )
                is_same = is_same and status
    return is_same
...
def nitf_file_diff(f1_name, f2_name, exclude=None, include=None, config=None):
    ...
    logger.debug(
        "Comparing %d file-level TREs" % min(len(f1.tre_list), len(f2.tre_list))
    )
    if len(f1.tre_list) != len(f2.tre_list):
        logger.error(
            "File 1 has %d file level TREs while file 2 has %d"
    ...
    status = _handle_type(
        f1.image_segment, f2.image_segment, "image segment", iseg_handle, logger
    )
    is_same = is_same and status

    logger.debug("Comparing text segments")
    status = _handle_type(
        f1.text_segment, f2.text_segment, "text segment", tseg_handle, logger
    )
    is_same = is_same and status

    logger.debug("Comparing des segments")
    status = _handle_type(
        f1.des_segment, f2.des_segment, "des segment", dseg_handle, logger
    )
    is_same = is_same and status
...
__all_ = [
    "DiffHandle",
    "DefaultHandle",
    "register_file_header_handle",
    "register_iseg_handle",
    "register_tre_handle",
    "register_tseg_handle",
    "register_dseg_handle",
    "nitf_file_diff",
]
